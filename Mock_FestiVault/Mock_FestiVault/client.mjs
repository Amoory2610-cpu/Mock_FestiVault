...
document.getElementById('searchFestivalButton').addEventListener('click', () => {

// Q2 complete these statements to read event title and organizer from form

    const title     = document.getElementById('festivalTitle').value;
    const organiser = document.getElementById('festivalOrganizer').value;

// Q3 complete the fetch API call to send the user's chosen title and
// organizer to the 'festival search' route in server.mjs.
    
    fetch('/searchFestival', {
        method: 'POST',
...
// data to the searchResults <div> in the format shown in the paper.
        ...
        json.forEach(event => {
            ...
            p.innerText = `Class – ${cls.className}\nInstructor – ${cls.instructor}\nCategory – ${cls.category}\nAvailable – ${cls.available ? 'Yes' : 'No'}`;
            
            // Q8 update with a "Reserve" button
            const btn = document.createElement('button');
            btn.innerText = 'Reserve';
            btn.addEventListener('click', () => {
                fetch('/reserveFestival', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
...
document.getElementById('addFestivalButton').addEventListener('click', async () => {

// Q9 complete these statements to read event details from the form

    const title     = document.getElementById('newFestivalTitle').value;
    const organiser = document.getElementById('newFestivalOrganizer').value;
    const category  = document.getElementById('newFestivalCategory').value;
    const available = document.getElementById('newFestivalAvailable').checked;

// Q9 complete the fetch API call to send the data to the 'add festival'
// route on the server as a POST request...

    const res = await fetch('/addFestival', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
